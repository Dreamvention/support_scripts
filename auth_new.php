...
if ($config->get('db_autostart') && $version >= '2.2.0.0') {
    ...
    if($version < '4.0.0.0') {
        ...
        $registry->set('db', new Opencart\System\Library\DB($engine_db, $config->get('db_hostname'), $config->get('db_username'), $config->get('db_password'), $config->get('db_database'), $config->get('db_port')));
    }
...
if($version >= '3.0.2.0' && $version < '4.0.0.0') {

    $session = new Session($config->get('session_engine'), $registry);

} elseif($version >= '4.0.0.0') {
...
if ($version >= '2.3.0.0' && $version < '4.0.0.0') {
    ...
    $registry->set('url', new Opencart\System\Library\Url($config->get('site_url'), $config->get('site_ssl')));
...
if (($registry->get('request')->server['REQUEST_METHOD'] == 'POST')) {
    ...
    if($version >= '3.0.0.0' && $version < '4.0.0.0') {
        ...
        $session->data['user_token'] = Helper\General\token(32);
        $url = $registry->get('url')->link('common/login', 'user_token='.$session->data['user_token']);
